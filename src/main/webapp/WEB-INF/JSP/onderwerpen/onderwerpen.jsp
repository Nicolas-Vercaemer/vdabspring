					...
					<form method="post" action="${verwijderURL}">
						<c:forEach items="${onderwerpen}" var="onderwerpUitLijst">
							<tr>
								...
								<td><c:if test="${onderwerpUitLijst.afwezig == true}">
Ja
</c:if></td>
								<td><spring:url value='/onderwerpen/bewerken/{id}'
										var='bewerkenURL'>
										<spring:param name="id" value="${onderwerpUitLijst.id}" />
									</spring:url> <input type="submit" value="${onderwerpUitLijst.id}" name="id"
									class="verwijderen" /> <a href="${bewerkenURL}"><img
				...
				<tfoot>
					<tr>
						<td colspan=3><c:if
								test="${empty onderwerp or not empty foutverwijderen}">
...
Ja
									...
									<td><spring:url value='/onderwerpen/bewerken/{id}'
											var='bewerkenURL'>
											<spring:param name="id" value="${onderwerpUitLijst.id}" />
										</spring:url> <input type="submit" value="${onderwerp.id}" name="id"
										class="verwijderen" /> <a href="${bewerkenURL}"><img
											alt="edit" src="${contextPath}/images/icons/pencil.png"></a></td>
								</tr>
							</c:otherwise>
